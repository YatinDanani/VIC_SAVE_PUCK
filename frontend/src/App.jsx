...
function Results({ data }) {
  ...
  const { meta, summary, modifiers, items, stands, timeline, watchlist } = data
    ...
    <div>
      ...
      <div className="fade-up" style={{
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:16 }}>
          <div>
            <div style={{ fontSize:10, color:'var(--ice-dim)', letterSpacing:'3px', textTransform:'uppercase', marginBottom:4 }}>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead>
              <tr style={{ borderBottom:'1px solid var(--border)' }}>
                {['Item','Category','Low','Forecast','High','Variance','Confidence'].map(h => (
    ...
    <div className="fade-in" style={{ display:'flex', flexDirection:'column', gap:16 }}>
      ...
      <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:14, padding:20 }}>
        ...
        <div style={{ fontSize:11, color:'var(--text-muted)', marginBottom:14 }}>
          Wednesday fans spend 12% more per head than average — Tuesday fans 12% less
        </div>
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg)', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ position:'fixed', inset:0, pointerEvents:'none', zIndex:0, opacity:1, animation:'rinkPulse 7s ease-in-out infinite' }}>
        <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
          ...
          {[['26%','34%'],['26%','66%'],['74%','34%'],['74%','66%']].map(([x,y],i) => (
            <g key={i}>
              <circle cx={x} cy={y} r="2.2%" fill="none" stroke="rgba(248,113,113,0.5)" strokeWidth="0.7"/>
              <line x1={`calc(${x} - 0.5%)`} x2={`calc(${x} + 0.5%)`} y1={y} y2={y} stroke="rgba(248,113,113,0.5)" strokeWidth="0.7"/>
              <line x1={x} x2={x} y1={`calc(${y} - 0.5%)`} y2={`calc(${y} + 0.5%)`} stroke="rgba(248,113,113,0.5)" strokeWidth="0.7"/>
            </g>
          ))}
        ...
        <header style={{ padding:'32px 0 24px', borderBottom:'1px solid var(--border)', marginBottom:32 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:16 }}>
            ...
            <nav style={{ display:'flex', gap:4 }}>
              {[['forecast','🏒 Forecast'], ['history','📊 History']].map(([v, label]) => (
              ...
              <div style={{
                ...
                <div>
                  ...
                  <span style={{ fontSize:11, opacity:0.7, marginTop:4, display:'block' }}>
                    Make sure the Python backend is running: <code>uvicorn main:app --reload</code>
                  </span>
                </div>
              </div>
            )}

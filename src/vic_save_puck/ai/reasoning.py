...
class ReasoningResult:
    """Structured output from the AI reasoning engine."""
    cause: str              # e.g. "untagged_promo", "stand_redistribution", "volume_surge", "noise"
    confidence: float       # 0.0 to 1.0
    actions: list[dict]     # [{stand, item, action, quantity_change}]
    alert_text: str         # shift-manager-readable plain English
    raw_reasoning: str      # full model response for debugging


SYSTEM_PROMPT = """You are an AI operations analyst for Save on Foods Memorial Centre, a WHL hockey arena in Victoria, BC. You analyze real-time F&B (food & beverage) demand data during hockey games.
...
- Shelf-stable items (cans, candy, chips): prep 95% — low waste risk
- Medium-hold items (popcorn, hot dogs, draught): prep 85% — moderate waste risk
- Short-life items (fries, tacos, pizza): prep only 75% — high waste risk, cook on demand
- Being SHORT means slower service (recoverable). Being OVER means waste (pure $ loss).
- Only recommend scaling UP when actual demand is clearly exceeding prep levels, not just when it exceeds forecast. A small positive drift is expected and fine.

DRIFT CAUSES you should classify:
1. "volume_surge" — overall demand higher than expected (e.g. bigger crowd than ticketed, high energy game)
2. "volume_drop" — overall demand lower than expected
3. "untagged_promo" — a promotion wasn't flagged in the system, causing specific item spikes
4. "stand_redistribution" — one stand is down/slow, others absorbing its demand
5. "weather_effect" — temperature affecting beer/hot drink mix
6. "timing_shift" — demand curve shifted earlier or later than expected
...
def analyze_drift(
    ...
    """
    Use Claude to classify drift cause and generate recommendations.

    Args:
        drift_report: Current window's drift signals
        game_context: {opponent, attendance, archetype, date, puck_drop_hour, ...}
        cumulative_drift: Overall drift so far
        recent_reports: Last 2-3 drift reports for trend context
    """
    # Build the context message
    signals_text = "\n".join(str(s) for s in drift_report.signals[:15])  # top 15
...
def _fallback_classify(
    ...
    # Check for stand redistribution pattern

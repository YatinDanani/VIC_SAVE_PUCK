...
class PromoRecommendation:
    """A recommended promotion or event."""
    recommendation_type: str  # "promo", "early_bird", "event", "scheduling"
    target_games: list[str]   # game dates or patterns
    description: str
    expected_impact: str
    confidence: float
    rationale: str
...
def analyze_promo_opportunities() -> list[PromoRecommendation]:
    """Analyze historical data to find promo/event optimization opportunities."""
    games = enrich_games()
    merged = load_merged()

    recommendations = []

    # ── 1. Identify low-attendance patterns ──────────────────────────────
    median_att = games["attendance"].median()
    low_att_games = games[games["attendance"] < median_att * 0.8]

    # Find patterns in low-attendance games
    ...
    if low_att_patterns:
        recommendations.append(PromoRecommendation(
            ...
            expected_impact=f"Could boost attendance 15-30% on weak days",
            confidence=0.75,
            rationale="\n".join(low_att_patterns),
        ))

    # ── 2. Analyze promo effectiveness ───────────────────────────────────
    ...
    # ── 3. Early-bird opportunity (slow pre-game starts) ────────────────
    ...
    # Games with low pre-game share are candidates for early bird
    low_pre = pre_ratio[pre_ratio["pre_pct"] < pre_ratio["pre_pct"].quantile(0.25)]
    if len(low_pre) > 3:
        low_pre_games = low_pre.merge(games[["game_date", "day_of_week", "puck_drop_hour"]], on="game_date")
        pattern = low_pre_games.groupby(["day_of_week", "puck_drop_hour"]).size().sort_values(ascending=False)

        recommendations.append(PromoRecommendation(
            recommendation_type="early_bird",
            target_games=pattern.index.tolist()[:3],
            ...
            confidence=0.7,
            rationale=f"{len(low_pre)} games have below-average pre-game sales ({pre_ratio['pre_pct'].quantile(0.25):.0%} cutoff). "
                      f"Top patterns: {pattern.head(3).to_dict()}",
        ))

    # ── 4. Per-cap spending by temperature band ─────────────────────────
    games["temp_band"] = pd.cut(
        games["temp_mean"], bins=[-10, 3, 8, 13, 20], labels=["Cold", "Cool", "Mild", "Warm"]
    )
    temp_qpc = games.groupby("temp_band", observed=True)["qty_per_cap"].mean()
    ...
    # ── 5. Intermission congestion analysis ─────────────────────────────
...
def generate_ai_event_recommendations(
    ...
    """Use Claude to synthesize data insights into actionable event strategy."""
